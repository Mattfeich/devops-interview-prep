...
**Откуда берутся коды вида 137 (128 + сигнал).** Exit code — один байт (0–255), а исходов два принципиально разных: процесс **сам** вышел через `exit(N)` (по договорённости 0–127) или его **убило ядро сигналом** — тогда своего кода у него нет вообще. Чтобы упаковать оба случая в одно число `$?`, договорились: убит сигналом → сообщаем **128 + номер сигнала** (128 — старший бит байта, взведён = «ниже номер сигнала, а не код возврата»; диапазоны не пересекаются). Шпаргалка: **130** = 128+2 (SIGINT, Ctrl+C), **137** = 128+9 (SIGKILL), **139** = 128+11 (SIGSEGV, segfault — баг приложения), **143** = 128+15 (SIGTERM, штатная остановка). Нюанс: 137 — не всегда OOM, это «убит SIGKILL», а кто послал — OOM killer, человек с `kill -9` или kubelet, добивший под, который не ответил на SIGTERM за terminationGracePeriod (30 с по умолчанию). Отличить: `kubectl describe pod` → Reason: OOMKilled, и `dmesg` на ноде.
